...
public class MidWalkEntityProcessingRunnable implements Runnable {
     ...
     */
    public MidWalkEntityProcessingRunnable(WalkGenerationManager generator, String entity, int numberOfWalks, int depth) {
        this.entity = entity;
    ...
    public void run() {
        if (walkGenerator.walkGenerator.getClass() == HdtWalkGenerator.class) {
            walkGenerator.writeToFile(((HdtWalkGenerator) walkGenerator.walkGenerator).generateMidWalksForEntity(walkGenerator.shortenUri(entity), this.numberOfWalks, this.depth));
            ...
            walkGenerator.writeToFile(((NtMemoryWalkGenerator) walkGenerator.walkGenerator).generateMidWalksForEntity(walkGenerator.shortenUri(entity),this.depth, this.numberOfWalks));
        } else if (walkGenerator.walkGenerator.getClass() == NxMemoryWalkGenerator.class) {
            walkGenerator.writeToFile(((NxMemoryWalkGenerator) walkGenerator.walkGenerator).generateMidWalksForEntity(walkGenerator.shortenUri(entity), this.depth, this.numberOfWalks));
